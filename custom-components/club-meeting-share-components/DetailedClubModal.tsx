import { StyleSheet, Pressable, Text, View, Animated, Easing, Image, ScrollView, TextInput, Button, KeyboardAvoidingView } from 'react-native';
...
import { GETmyAcceptedClubs, GETreceivedClubsInNotification, GETUserEmail } from '@/utils/helper-functions/GetSet_UserInfo';
...
const DetailedClubModal = ({ clubName, setIsClubDetailModalVisible, isClubDetailModalVisible }: DetailedClubModalProps) => {
    ...
    const [daysForClub, setDaysForClub] = useState<string[] | null>(null);
    //not used yet but potential use is to add start and end dates option and make one time weekly meetings
    ...
    const checkExistingMeeting = async () => {
        ...
        if (existingAcceptedMeeting || existingReceivedMeeting) {
            const existingMeetingType = existingAcceptedMeeting ? 'schedule' : 'notifications';
            ...
            const hours = ((endTime.getTime() - startTime.getTime()) / (60 * 60 * 1000)).toFixed(2);
    ...
    const sendMeeting = async () => {
        ...
        updateClubInRealtimeDatabase(clubData);
            ...
            >
                ...
                <View style={{ backgroundColor: '#434343', padding: 20, borderTopLeftRadius: 20, borderTopRightRadius: 20, height: '95%', }}>
                    ...
                    <View style={{ flex: 1, flexDirection: 'column', justifyContent: 'space-between' }}>
                        ...
                        <View style={{}}>
                            <Pressable onPress={checkExistingMeeting} style={{ backgroundColor: '#e86464', padding: 15, borderRadius: 10, alignItems: 'center', justifyContent: 'center', marginBottom: 10 }}>
                                <Text style={{ color: 'white', fontWeight: '500', fontSize: 17 }}>Share with all members</Text>
                            </Pressable>
                        </View>
